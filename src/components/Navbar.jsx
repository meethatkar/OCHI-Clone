...
const Navbar = () => {
    ...
    return (
        <nav className='fixed z-[999] flex items-center justify-between w-full py-5 px-[4vw] font-["Test Founders Grotesk X-Cond-SmBd"] backdrop-blur-md'>
            <div id="logo">
            ...
            <div id="nav-btns" className='flex gap-10'>
                {nav_btns.map((item, idx) => (
                    <a
                        href="#"
                        key={idx}
                        className={`text-lg font-normal ${idx === (nav_btns.length - 1) ? "ml-[10vmax]" : ""} nav-link group`}
                    >
                        {[...item].map((char, j) => (
                            <span key={j} className='nav-letter inline-block'>{char}</span>
                        ))}
                    </a>
                ))}
            </div>
